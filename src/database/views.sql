...
CREATE VIEW FP.V_GERAL AS
	SELECT		FP.Evento.nome AS EVENTO_NOME, FP.Evento.dataInicio, FP.Evento.nDias, FP.Pessoa.nome AS PROMOTOR_NOME, FP.Pessoa.email AS PROMOTOR_EMAIL, FP.Pessoa.telefone AS PROMOTOR_TELEFONE, FP.Banda.nome
	FROM		FP.Pessoa INNER JOIN
				FP.Promotor ON FP.Pessoa.cc = FP.Promotor.cc INNER JOIN 
				FP.Evento ON FP.Promotor.cc = FP.Evento.cc_promotor INNER JOIN
				FP.Concerto	ON FP.Concerto.id_evento = FP.Evento.id INNER JOIN
